...
// OpenIDConnectProvider manages AWS Identity and Access Management
...
type OpenIDConnectProvider struct {
	...
	// format for a client ID. The CreateOpenIDConnectProviderRequest operation
	// accepts client IDs up to 255 characters long.
	ClientIDList []string `input:"client_id_list"`
	...
	// one thumbprint when creating an IAM OIDC provider. For example, assume
	// that the OIDC provider is server.example.com and the provider stores its
	// keys at https://keys.server.example.com/openid-connect. In that case,
	// the thumbprint string would be the hex-encoded SHA-1 hash value of the
	// certificate used by https://keys.server.example.com.For more information
	// about obtaining the OIDC provider's thumbprint, see Obtaining the
	...
	ThumbprintList []string `input:"thumbprint_list"`

	// The URL of the identity provider. The URL must begin with https:// and
	// should correspond to the iss claim in the provider's OpenID Connect ID
	// tokens. Per the OIDC standard, path components are allowed but query
	// parameters are not. Typically the URL consists of only a hostname, like
	// https://server.example.org or https://example.com.You cannot register
	// the same provider multiple times in a single AWS account. If you try to
	// submit a URL that has already been used for an OpenID Connect provider
	// in the AWS account, you will get an error.
	URL string `input:"url" json:"Url"`
	...
	OpenIDConnectProviderARN *string `json:"OpenIDConnectProviderArn" output:"open_id_connect_provider_arn"`
}
